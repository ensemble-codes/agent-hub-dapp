...
export class SocketIOManager extends EventAdapter {
  ...
  public initialize(clientEntityId: string, communicationURL: string, agentIds: string[], namespace: string = '/'): void {
    ...
    this.socket.on('connect', () => {
      ...
      if (process.env.NODE_ENV === 'development' && this.socket) {
        this.socket.onAny((event, ...args) => {
          console.debug(`[SocketIO DEBUG] Received event '${event}':`, args);
        });
      }
    ...
    this.socket.on('messageBroadcast', (data: MessageBroadcastData) => {
      ...
      if (channelId && this.activeChannelIds.has(channelId)) {
        console.info(`[SocketIO] Handling message for active channel ${channelId}`);
    ...
    this.socket.on('controlMessage', (data) => {
      ...
      const channelId = data.channelId || data.roomId; // Handle both new and old message format
      if (channelId && this.activeChannelIds.has(channelId)) {
        console.info(`[SocketIO] Handling control message for active channel ${channelId}`);
    ...
    this.socket.on('messageDeleted', (data) => {
      console.debug(`[SocketIO] Message deleted event received:`, data);

      // Check if this is for one of our active channels
      const channelId = data.channelId || data.roomId; // Handle both new and old message format
      if (channelId && this.activeChannelIds.has(channelId)) {
        console.info(`[SocketIO] Handling message deletion for active channel ${channelId}`);
    ...
    this.socket.on('channelCleared', (data) => {
      console.info(`[SocketIO] Channel cleared event received:`, data);

      // Check if this is for one of our active channels
      const channelId = data.channelId || data.roomId; // Handle both new and old message format
      if (channelId && this.activeChannelIds.has(channelId)) {
        console.info(`[SocketIO] Handling channel cleared for active channel ${channelId}`);
    ...
    this.socket.on('channelDeleted', (data) => {
      console.info(`[SocketIO] Channel deleted event received:`, data);

      // Check if this is for one of our active channels
      const channelId = data.channelId || data.roomId; // Handle both new and old message format
      if (channelId && this.activeChannelIds.has(channelId)) {
        console.info(`[SocketIO] Handling channel deleted for active channel ${channelId}`);
    ...
    this.socket.on('reconnect_attempt', (attempt) => {
      console.info('[SocketIO] Reconnect attempt', attempt);
      this.emit('reconnect_attempt', attempt);
    });

    this.socket.on('reconnect', (attempt) => {
      console.info(`[SocketIO] Reconnected after ${attempt} attempts`);
    ...
    this.socket.on('log_stream', (data) => {
      console.debug('[SocketIO] Log stream data received:', data);
    ...
    this.socket.on('log_subscription_confirmed', (data) => {
      console.info('[SocketIO] Log subscription confirmed:', data);
  ...
  public async subscribeToLogStream(): Promise<void> {
    if (!this.socket) {
      console.error('[SocketIO] Cannot subscribe to logs: socket not initialized');
      return;
    }
    ...
    console.info('[SocketIO] Subscribed to log stream');
  }
  ...
  public async unsubscribeFromLogStream(): Promise<void> {
    if (!this.socket) {
      console.error('[SocketIO] Cannot unsubscribe from logs: socket not initialized');
      return;
    }
    ...
    console.info('[SocketIO] Unsubscribed from log stream');
  }
  ...
  public async updateLogStreamFilters(filters: {
    ...
    if (!this.socket) {
      console.error('[SocketIO] Cannot update log filters: socket not initialized');
      return;
    }
    ...
    console.info('[SocketIO] Updated log stream filters:', filters);
  }
